...
excerpt: |
  A practical guide for Product Managers and Product Designers on writing good, reusable AI "skills," the instruction sets that make an agent do the same job well, every time. Built on Matt Pocock's four-part framework, reframed through a product lens, with one complete skill you can lift and use this week.
...
That paragraph was a skill trying to be born. I just hadn't written it down properly yet. 

If you've encountered this issue or never written a skill, I'm going to show you how to do so here.

{% include sidebar.html heading="What's a skill?" text="A **skill** is a reusable, version-controlled instruction set that an agent loads on demand, a document that says \"here's how we do this particular job, and here's what good looks like.\" Because it's just structured markdown, the same skill works across Claude (Skills, Projects, Claude Code) and OpenAI (Custom GPTs, Projects, a saved system prompt). Write it once, and every teammate who runs it gets your best version of the task instead of their improvised one." %}
...
The tips in this article are influeneced by the work of **Matt Pocock**, whose "Missing Manual: How to Write Great Skills" talk gives the cleanest framework I've found for the problem. Matt maintains one of the most-used engineering skill sets out there, and his [skills repo](https://github.com/mattpocock/skills) is worth your time. His own `grill-with-docs` skill is part of my daily workflow. The four-part spine below (**Trigger, Structure, Steering, Pruning**) is entirely his. What's mine is the translation: Matt pitched this to engineers, and I want to hand it to the Product Managers and Product Designers I work with. So the framework is his; the product lens, the examples, and the mistakes are mine.
...
There are two ways a skill can fire. **Deliberate** invocation is when *you* call it. You type its name, pick it from a menu, or paste it in. **Automatic** invocation is when the *agent* decides to load it, based on a short description that always sits in its context. The agent reads that description, thinks "this fits," and pulls the skill in on its own.
...
Either way, every skill you write gets a one-line **description**. For an automatic skill that line does real work: it's the trigger the agent reads to decide whether to fire, so write it to discriminate. For a deliberate skill the same line is just a label that reminds *you* what the skill is for. If you're on Claude and want to guarantee a skill stays deliberate, add `disable-model-invocation: true` so the description only ever shows to you, never to the agent. You'll see that description line at the top of every example below.
...
The constraint that keeps a skill effective is this: **keep the main skill file as small as possible.** Small skills are easier to read, easier to maintain, and cheaper to run. Every word you cut is tokens saved on every use. Designers already know this instinct. It's the same discipline as a design system: you don't dump every one-off pattern into the core library.
...
description: Review a design for usability or accessibility issues.
---
...
The checklists live in `heuristics.md` and `a11y-checklist.md`, bundled alongside the skill, pulled in only when that branch runs. If your skill only ever does one thing, skip this; one branch means all the reference belongs in the file. Branching is the tool for skills that do *several* things.
...
The obvious fix (put "recommend" in a later step and tell the agent to wait) doesn't fully work, because the agent still *reads* that later step before it starts the first one. The finish line is right there on the page; it just knows to sprint to it. Matt's fix is more radical: when leg work matters more than anything, don't separate the *steps*. Separate the *skills.* He refused to build a two-step "plan mode" for exactly this reason: "ask clarifying questions" always got shortchanged because "make the plan" was sitting visible right below it. So he split it into two skills (`grill-with-docs`, then `to-PRD`) and the agent only ever sees the step it's on.
...
- **Repetition.** Every piece of a skill should have a single source of truth. If your definition of "an insight" appears in three places, you'll update one and leave the other two stale. This is design-system stewardship applied to prose: one canonical definition, referenced, never copied.
...
- **No-ops.** These are lines that *look* like they're doing something but change nothing. The test is simple. **Delete it and see.** If you remove "write a clear, thorough summary" and the summary comes out just as good, that line was a no-op. Agents (and well-meaning teammates) love adding these. Delete-and-check is how skills stay small.
...
- **Pruning.** Delete every line that survives the delete-and-check test unchanged. Give it a single source of truth. Retire it when it goes stale.
